...
"""
   Determines if the source and target of a taxi trip is within the network bounds.

   Parameters
   -----------
   t: Int64 
      The trip index for which we wish to determine if it is within the network bounds.

   Returns
   within_bounds: Bool
      If true the source and target of trip t are within the bounds of the network defined
      by the maximum and minimum lattitudes and longitudes of the network.
"""
...
"""
   Prepares taxi requests data for precomputation of taxi trips.
   
   NOTES
   -----
   -Extracts requests that are within the network bounds.
   -Extracts requests with source and target locations within 250m of a network node.
   -Finds closest nodes in network to source and target locations.
   -Assigns week to each request (cycles through weeks 1-4).
   -Computes simulation time of each trip request (accurate to the minute).
   -WAIT, the time between the current request and the previous request.
"""
# =================================================================
# LOAD DATA
...
jldopen("data/network-karlsruhe.jld2") do fn
...
open("data/ranks-karlsruhe.csv") do fn
   data = CSV.File(fn; normalizenames=true) |> DataFrame
...
open("data/TripsKA_SharedWithUoA.csv") do fn
   ...
   for t = 1:num_trips
      SLAT[t] = ratio * data[t,3]
      SLON[t] = ratio * data[t,2]
      TLAT[t] = ratio * data[t,5]
      TLON[t] = ratio * data[t,4]
      DAY[t] = data[t,9]
      HOUR[t] = data[t,10]
      MIN[t] = data[t,11]
   end
...
for t = 1:num_trips
   ...
   for v = 1:num_nodes
     vlat = LAT[v]
     vlon = LON[v]
     ratio = sin(slat) * sin(vlat) + cos(slat) * cos(vlat) * cos(slon - vlon)
     if (ratio>1)
         ratio = 1
         println("Invalid Ratio")
     end
     d = radius * acos(ratio)
     if d < sdist
         snode = v
     ...
     ratio = sin(tlat) * sin(vlat) + cos(tlat) * cos(vlat) * cos(tlon - vlon)
     if (ratio>1)
         ratio = 1
         println("Invalid Ratio")
     end
   ...
   for r = 1:num_ranks
      ...
      rlat = LAT[rnode]
      rlon = LON[rnode]
      ratio = sin(slat) * sin(rlat) + cos(slat) * cos(rlat) * cos(slon - rlon)
      if (ratio>1)
         ratio = 1
         println("Invalid Ratio")
      end
   ...
   if (snode != tnode) & (sdist<250) & (tdist<250)
      push!(useful2, t)
...
for t = 2:num_trips
   ...
   FINAL[t,9] = sum(FINAL[:,6])
end

names = ["ID", "WEEK", "DAY", "HOUR", "MINUTE", "WAIT", "SOURCE", "TARGET", "TIME"]
df = DataFrame(FINAL, Symbol.(names))
CSV.write("test-calls.csv", df)
